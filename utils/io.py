...
def config_to_dataloader(opts, is_eval=False):
    ...
    """
    # TODO: Sample here neighborhood from different images (then can use locally coherent img/sil losses)
...
def get_vertex_colors(model, mesh, frame_idx=0, view_dir=None, fine_nerf_net=False, vertices=None):
    ...
    #xyz_embedded = torch.cat([xyz_embedded, env_code],-1)ç
    if fine_nerf_net:
        vis = model.nerf_fine(xyz_embedded)[:,:3].cpu().numpy()
    else:
        vis = model.nerf_coarse(xyz_embedded)[:,:3].cpu().numpy()
    vis = np.clip(vis, 0, 1)
    return vis

def save_tensor_as_img(tensor_imgs, fig_name="figures/tmp"):
    imgs = (tensor_imgs.permute(0, 2, 3, 1)*255).detach().cpu().numpy().astype(np.uint8)
    for i in range(len(imgs)):
        img = imgs[i][:,:,::-1]
        cv2.imwrite(fig_name+"_"+str(i)+".jpg", img)
    
def save_mesh_trimesh(mesh_pytorch3d, fig_name="figures/tmp.obj"):
    mesh_trimesh = trimesh.Trimesh(vertices=mesh_pytorch3d.verts_packed().detach().cpu().numpy(), 
                        faces=mesh_pytorch3d.faces_packed().detach().cpu().numpy(), 
                        vertex_normals=mesh_pytorch3d.verts_normals_packed().detach().cpu().numpy(),
                        face_normals=mesh_pytorch3d.faces_normals_packed().detach().cpu().numpy())
    # TODO: Add color again here
    vertex_colors=mesh_pytorch3d.textures.verts_features_packed().detach().cpu().numpy()*255
    mesh_trimesh.visual.vertex_colors[:,:3] = vertex_colors
    mesh_trimesh.export(fig_name)
