...
class Embedding(nn.Module):
    def __init__(self, in_channels, N_freqs, logscale=True, alpha=None, nerfies_window=False):
        """
        ...
        self.nerfies_window = nerfies_window
...
class EmbeddingDFMPose(nn.Module):
    def __init__(self, pose_emb, dfm_emb, dim_pose_emb, dim_dfm_emb, output_length):
        super(EmbeddingDFMPose, self).__init__()
        self.pose_emb = pose_emb
        self.dfm_emb = dfm_emb
        self.dim_pose_emb = dim_pose_emb
        self.dim_dfm_emb = dim_dfm_emb
        self.output_length = output_length

        self.comb_layer = nn.Linear(self.dim_pose_emb+self.dim_dfm_emb, self.output_length)

    def forward(self, frameid):
        f_pose_emb = self.pose_emb(frameid) #.detach()
        f_dfm_emb = self.dfm_emb(frameid)
        emb = torch.cat([f_pose_emb, f_dfm_emb], -1)
        res_emb = self.comb_layer(emb)
        return res_emb
...
class NeRF(nn.Module):
    def __init__(self,
                 ...
                 out_channels=3, 
                 skips=[4], raw_feat=False, init_beta=1./100, large_cond_embedding=False,
                 activation=nn.ReLU(True), in_channels_code=0, use_sdf_to_density=False):
        """
        adapted from https://github.com/kwea123/nerf_pl/blob/master/models/nerf.py
        ...
        self.skips = skips
        self.use_xyz = False
        self.large_cond_embedding = large_cond_embedding
        ...
        if not self.large_cond_embedding:
            self.dir_encoding = nn.Sequential(
                                    nn.Linear(W+in_channels_dir, W//2),
                                    activation)
        else:
            # nerf paper description of network
            layers = [nn.Linear(in_channels_dir + W, W//2), activation]
            for _ in range(D//2):
                layers = layers + [nn.Linear(in_channels_dir + W, W//2), activation]
            self.dir_encoding = nn.Sequential(*layers)
        ...
        self.use_sdf_to_density = use_sdf_to_density
        if use_sdf_to_density:
            layers = [nn.Linear(1 + W, W//8), activation]
            for _ in range(D//2):
                layers = layers + [nn.Linear(W//8, W//8), activation]
            layers.append(nn.Linear(W//8, 1))
            self.sdf_to_density = nn.Sequential(*layers)
...
#                    nn.init.xavier_uniform_(m.weight)
    ...
    def forward(self, x ,xyz=None, sigma_only=False):
        ...
        if not sigma_only:
            ...
            if x.ndim==3:
                x = torch.cat([x, torch.zeros(x.size(0), x.size(1), self.in_channels_xyz-x.size(2)).to(x.get_device())], 2)
            else:
                x = torch.cat([x, torch.zeros(x.size(0), self.in_channels_xyz-x.size(1)).to(x.get_device())], 1)
        ...
        if not self.use_sdf_to_density:
            ...
            sdf = self.sigma(xyz_)
            sigma = self.sdf_to_density(torch.cat([sdf, xyz_], -1))
            if sigma_only:
                return sigma
...
class ExtraDFMMLP(nn.Module):
    def __init__(self, n_input_dims, n_layers=5, n_hidden_dim=128, output_dim=3):
        super(ExtraDFMMLP, self).__init__()
        self.n_layers=n_layers
        self.n_hidden_dim=n_hidden_dim
        self.output_dim=output_dim
        assert n_layers > 2, "Deformation MLP is too small"

        layers = [nn.Linear(n_input_dims, n_hidden_dim), nn.ReLU()]
        for i in range(n_layers-2):
            layers.append(nn.Linear(n_hidden_dim, n_hidden_dim))
            layers.append(nn.ReLU())
        layers.append(nn.Linear(n_hidden_dim, output_dim))
