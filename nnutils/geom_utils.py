...
def cast_rays_means_covs_cone(t_samples, origins, directions, radii, diagonal=True):
...
def bone_transform(bones_in, rts, is_vec=False):
    ...
    rts: ...,B,3,4    - B rigid transforms
    rts are applied to bone coordinate transforms (left multiply)
    is_vec:     whether rts are stored as r1...9,t1...3 vector form
    """
    B = bones_in.shape[-2] # Bone coordinates defined in R^10
    bones = bones_in.view(-1,B,10).clone()
    if is_vec: # rts are rigid transformations to apply to bones
        rts = rts.view(-1,B,12)
    else:
        rts = rts.view(-1,B,3,4)
    bs = rts.shape[0] 
    # Bones defined as gaussian ellipsoids with center, orientation and scale (sec 3.2 Skinning weights)
    center = bones[:,:,:3]
    orient = bones[:,:,3:7] # orientation in quaternions
    scale =  bones[:,:,7:10]
    if is_vec:
    ...
    # move bone coordinates (left multiply) -> Apply rigid transformation to bones
    center = Rmat.matmul(center[...,None])[...,0]+Tmat[...,0] # Modify camera center
    Rquat = transforms.matrix_to_quaternion(Rmat)
    orient = transforms.quaternion_multiply(Rquat, orient) # Modify bone orientation

    scale = scale.repeat(bs,1,1) # Scale remains the same after rigid transformation
...
def gauss_mlp_skinning(xyz, embedding_xyz, bones, 
                    pose_code,  nerf_skin, chunk_mult_eval_mlp=8, skin_aux=None):
    """
    xyz:        N_rays, ndepth, 3
    bones:      ... nbones, 10
    pose_code:  ...,1, nchannel (time_embedded: It's just a latent code for each frame) 
    """
    N_rays = xyz.shape[0] # xyz N_rays, ndepth, 3
    ...
    xyz_embedded = embedding_xyz(xyz) # N_rays x N_depth x 63
    dskin = mlp_skinning(nerf_skin, pose_code, xyz_embedded, chunk_mult_eval_mlp=chunk_mult_eval_mlp) # If you want to compute delta skinning matrix
    skin = skinning(bones, xyz, dskin, chunk_mult_eval_mlp=chunk_mult_eval_mlp, skin_aux=skin_aux) # bs, N, B
    return skin

def mlp_skinning(mlp, code, pts_embed, chunk_mult_eval_mlp=8):
    """
    ...
    if mlp is None:
        ...
        dskin = evaluate_mlp(mlp, pts_embed, code=code, chunk=chunk_mult_eval_mlp*1024)
    return dskin
...
#def skinning(bones, pts, dskin=None, skin_aux=None):
    """
    bone: bs,B,10  - B gaussian ellipsoids (deformed bones)
    pts: bs,N,3    - N 3d points, usually N=num points per ray, b~=2034
    skin: bs,N,B   - skinning matrix
    """
    device = pts.device
    log_scale= skin_aux[0] # skin_aux is a parameter for something
    w_const  = skin_aux[1]
    bs,N,_ = pts.shape
    B = bones.shape[-2]
    ...
    center, orient, scale = vec_to_sim3(bones) # sim3: Similarity transformation in R^3
    ...
    if True:#B<50:
        ...
        mdis = mdis.pow(2)
    mdis = mdis*100*log_scale.exp() # TODO account for scaled near-far plane ?
    mdis = (-10 * mdis.sum(3)) # bs,N,B  # Mahalanobis distance of each point to each bone (N_rays x N_samples x N_bones)

    if dskin is not None: # Add delta weights if computed
        mdis = mdis+dskin
    ...
    skin = mdis.softmax(2) # Softmax -> Prob of each point to belong to the i-th bone
    return skin
    

def skinning(bones, pts, dskin=None, chunk_mult_eval_mlp=8, skin_aux=None):
    """
    bone: ...,B,10  - B gaussian ellipsoids
    pts: bs,N,3    - N 3d points
    skin: bs,N,B   - skinning matrix
    """
    chunk= max(1, chunk_mult_eval_mlp//2) * 1024
    bs,N,_ = pts.shape
    B = bones.shape[-2]
    if bones.dim()==2: bones = bones[None].repeat(bs,1,1)
    bones = bones.view(-1,B,10)
...
def raycast(xys, Rmat, Tmat, Kinv, near_far):
    ...
    xy1s_offx = xy1s.clone()
    xy1s_offx[:,:,0] = xy1s_offx[:,:,0] + 1
    xyz3d_offx = xy1s_offx.matmul(Kinv.permute(0,2,1))
    ray_directions_offx = xyz3d_offx.matmul(Rmat)
    ...
    try:
        use_radii = True
        ray_directions_offx_normz = ray_directions_offx / ray_directions_offx.norm(2, -1)[:,None]
        ray_directions_normz = ray_directions / ray_directions.norm(2, -1)[:,None]
        dx = torch.sqrt(torch.sum((ray_directions_offx_normz - ray_directions_normz) ** 2, -1))
        # Cut the distance in half, and then round it out so that it's
        # halfway between inscribed by / circumscribed about the pixel.
        radii = dx*2 / 12**0.5 #[v[..., None] * 2 / np.sqrt(12) for v in dx]
        radii = radii[:,:,None].detach()
    except:
        ray_directions_offx_normz = ray_directions_offx / ray_directions_offx.norm(2, -1)[:,:,None]
        ray_directions_normz = ray_directions / ray_directions.norm(2, -1)[:,:,None]
        dx = torch.sqrt(torch.sum((ray_directions_offx_normz - ray_directions_normz) ** 2, -1))
...
def sample_xy(img_size, bs, nsample, device, consecutive_line_sampler=False, return_all=False, lineid=None):
    """
    rand_inds:  bs, ns
    xys:        bs, ns, 2
    """ # nsample add as parameter
    xygrid = np.meshgrid(range(img_size), range(img_size))  # w,h->hxw
    xygrid = torch.Tensor(xygrid).to(device)  # (x,y) # TODO: check this (pytorch userwarning)
    ...
    if return_all:
        ...
        else: # Here randomly selects a column in the given loaded lines (in lineid)

            if consecutive_line_sampler: # Default random sampling
                probs = torch.ones(img_size).to(device) # 512*512 vs 128*64
                cols = torch.randint(nsample//2, img_size-nsample//2-1, (bs//nsample,))
                rand_inds = torch.cat([torch.arange(col-nsample//2, 
                                math.ceil(col+nsample/2)).unsqueeze(0).repeat((nsample, 1)) for col in cols]).to(device)
            else:
                probs = torch.ones(img_size).to(device) # 512*512 vs 128*64
...
def generate_bones(num_bones_x, num_bones, bound, device):
    ...
    center =torch.meshgrid(center, center, center) # num_bones x num_bones x num_bones centers ?
    ...
    bones = torch.cat([center, orient, scale],-1) # Bones parametrized by center, orientation and scale -> Used to compute "attachment" weights of each point to each bone 
    return bones
...
def correct_rest_pose(opts, bone_rts_fw, bone_rts_rst):
    # delta rts
    reshaped = False
    if bone_rts_fw.ndim == 2:
        reshaped = True
        bone_rts_fw = bone_rts_fw.unsqueeze(0)
    ...
    if reshaped:
        bone_rts_fw = bone_rts_fw.squeeze(0)
...
def warp_fw(opts, model, rt_dict, vertices, embedid, maintain_grad=False):
    """
    only used in mesh extraction
    """
    ...
    if not maintain_grad:
        pts_can=torch.Tensor(vertices).to(model.device)
    else:
        pts_can = vertices.to(model.device)
    ...
    if not maintain_grad:
        vertices = pts_dfm.detach().cpu().numpy()
    else:
        vertices = pts_dfm
    
    return vertices, rt_dict

def warp_meshes(opts, model, rt_dict, mesh, embedid):
    """
    takes as input the canonical mesh and applies forward mapping to generate meshes at different time instances
    """
    pts_can = mesh.verts_packed()
    num_pts = pts_can.shape[0]

    # First correct mesh and compute skinning weights for canonical mesh
    pts_can = pts_can[:,None]
    bones_rst = model.bones
    bones_rst, bone_rts_rst = correct_bones(model, bones_rst)
    ...
    # Deform each mesh: forward skinning
    finer_meshes = mesh.extend(embedid.size(0))
    n_meshes = len(finer_meshes)
    query_time = torch.ones(n_meshes,1).long().to(model.device)*embedid.unsqueeze(1)
    bone_rts_fw = model.nerf_body_rts(query_time)
    bone_rts_fw = correct_rest_pose(opts, bone_rts_fw, bone_rts_rst)
    
    #bone_rts_fw_pts = bone_rts_fw[i_mesh].unsqueeze(0).expand((n_meshes, -1, -1))
    pts_can = pts_can.permute(1, 0, 2).expand((n_meshes, -1, -1))
    bones_rst = bones_rst.unsqueeze(0) #.expand((n_meshes, -1, -1))
    skin_forward = skin_forward.permute(1, 0, 2).expand((n_meshes, -1, -1))
    pts_dfm,bones_dfm = lbs(bones_rst, bone_rts_fw, skin_forward, pts_can,backward=False)
    pts_dfm = pts_dfm.resize(pts_dfm.size(0)*pts_dfm.size(1), 3)

    lbs_offset = pts_dfm - finer_meshes.verts_packed()
    # Deformed mesh at frame t
    finer_meshes = finer_meshes.offset_verts(lbs_offset)
...
def get_near_far(near_far, vars_np, tol_fac=1.2, pts=None):
    ...
    if pts is None:
        ...
        if vars_np["mesh_rest"].vertices.shape[0] < 100:
            return near_far
...
def ood_check_cse(dp_feats, dp_embed, dp_idx):
    ...
    for i in range(bs):
        ...
        rpj_coord = torch.stack([rpj_idx % w, rpj_idx//w],-1) # TODO: check this (pytorch userwarning)
...
def subdivide_mesh_py3d(mesh, times=1):
    if times <= 0:
        return mesh
        
    subdivider = SubdivideMeshes()
    n_verts = mesh.textures.faces_verts_textures_packed().size(0)*mesh.textures.faces_verts_textures_packed().size(1)
    mesh, cols = subdivider(mesh, mesh.textures.faces_verts_textures_packed().resize(n_verts, 3))
    for i in range(times):
        mesh, cols = subdivider(mesh, cols)
    textures = Textures(verts_rgb=cols.resize(1, cols.size(0), cols.size(1)))
