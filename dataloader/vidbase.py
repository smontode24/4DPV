...
class BaseDataset(Dataset):
    ...
    def read_raw(self, im0idx, flowfw,dframe):
        ...
        try: # Read and reshape optical flow and occ(?) to match the size of the image and mask
        ...
        flow = flow[...,:2]
        occ[occluder] = 0 # What is this?
        
        #print('flo:%f'%(time.time()-ss))
        # Now read DensePose CSE surface embeddings (for dense keypoint matching)
        try:
        ...
        try:
            dp_feat = readPFM(self.featlist[im0idx])[0] # DensePose CSE features 16x112x112
            dp_bbox =  np.loadtxt(self.bboxlist[im0idx]) # Bounding box where the object is located
        except: 
            print('error loading densepose feature')
            dp_feat =  np.zeros((16*112,112))
            dp_bbox =  np.zeros((4))
        dp= (dp *50).astype(np.int32)
        dp_feat = dp_feat.reshape((16,112,112)).copy() # From here we predict with ResNet18 camera pose (second stage of PoseNet)
        
        #print('dp:%f'%(time.time()-ss))

        # add RTK: [R_3x3|T_3x1]
        #          [fx,fy,px,py], to the ndc space
        try: # Load camera parameters if they are available
            ...
            rtk[3, :]  = np.asarray([512,512,256,256])  # What is this last row?
        ...
        if not self.dont_crop:
            ...
            hp0, kaug, A, B = None, None, None, None
        ...
        rt_dict = {}                               # Everything is cropped to same size and only taking into account the region where the cat is
        rt_dict['img']   = img                     # img is of size 64x64, at least in first optimization step
        rt_dict['mask']  = mask                    # cat segmentation mask -> Why it contains different values from 0 and 1 (1,2,3,4,5,...,15)
        rt_dict['flow']  = flow                    # Optical flow
        rt_dict['occ']   = occ                     # Occlusion of which parts become occluded in next frame ?
        rt_dict['dp']    = dp                      # ? 
        rt_dict['vis2d'] = vis2d                   # create mask for visible vs unkonwn
        rt_dict['dp_feat'] = dp_feat               # DensePose 16x112x112 features
        rt_dict['dp_feat_rsmp'] = dp_feat_rsmp     # DensePose 16x112x112 features -> Resized to 1x16x64x64
        rt_dict['dp_bbox'] = dp_bbox               # Bbox of cat/human
        rt_dict['rtk'] = rtk                       # 4x4 camera parameters [R|t] + [fx, fy, px, py] -> Not known estimate with PoseNet
    ...
    def load_data(self, index):
        ...
        if self.load_pair:  # Load also next frame info if necessary
        ...
        elem['vis2d']         =  vis2d      # y
        elem['rtk']           =  rtk
        elem['kaug']          =  kaug       # Arbitrary intrinsic parameters for cropped image of cat
        elem['dataid']        =  dataid     # diff dataid and frameid?
        elem['frameid']       =  frameid
        elem['is_canonical']  =  is_canonical # A canonical frame can be set for shape at rest? Boolean
        return elem
