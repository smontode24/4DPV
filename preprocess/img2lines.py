...
def main(_):
    ...
    data_info = trainer.init_dataset() # Get dataset info from that sequence
    ...
    dataset = config_to_dataloader(opts_dict,is_eval=True) # Read video dataset and concatenate it to a single dataset
    ...
    os.makedirs("inp_imgs", exist_ok=True)
    ...
    for i, batch in tqdm(enumerate(dataset)):
        ...
        dataid = batch['dataid']
        dt = frameid[0,1] - frameid[0,0] # Load two consecutive frames?
        frameid = frameid + data_offset[dataid[0,0].long()] # Add data offset?
        ...
        if frameid[0,0].item() < 10:
            cv2.imwrite("inp_imgs/%04d.jpg"%(frameid[0,0].item()), (dict_array["img"][0,0].transpose(1,2,0) * 255).astype(np.uint8)[:,:,::-1])

        np.save(save_path_rtk, dict_rtk)

        for idy in range(opts.img_size): # Save each row of size ( 1 (batch size), 2 (two consecutive frames I think), 3 (RGB), 512 (width) )
            save_path = '%s/%04d.npy'%(save_dir_t, idy)
            dict_px = dict2pix(dict_array, idy) 
            np.save(save_path, dict_px) # Not only saves row, also all the other information
